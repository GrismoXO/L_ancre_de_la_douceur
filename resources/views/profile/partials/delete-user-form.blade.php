<section class="space-y-6">
    <header>
        ...
        <p class="mt-1 text-sm text-gray-600 dark:text-gray-400 open">
    ...
    <x-danger-button
        x-data=""
        x-on:click.prevent="$dispatch('open-modal', 'confirm-user-deletion')" class="kalam hover"
    ...
    <x-modal name="confirm-user-deletion" :show="$errors->userDeletion->isNotEmpty()" focusable>
        <form method="post" action="{{ route('profile.destroy') }}" class="p-6">
            ...
            <h2 class="text-lg font-medium text-gray-900 dark:text-gray-100 open">
            ...
            <div class="mt-6">
                <x-input-label for="password" value="{{ __('Mot de passe') }}" class="sr-only open" />

                <x-text-input
                    id="password"
                    name="password"
                    type="password"
                    class="mt-1 block w-3/4 open"
            ...
            <div class="mt-6 flex justify-end">
                <x-secondary-button x-on:click="$dispatch('close')" class="kalam">
                    {{ __('Annuler') }}
                </x-secondary-button>

                <x-danger-button class="ml-3 kalam">
                    {{ __('Supprimer le compte') }}
                </x-danger-button>
            </div>
        </form>
