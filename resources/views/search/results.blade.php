<x-app-layout>
  ...
  <div class="w-full pt-4 pb-8 nav_barre bg_pink_opa">
    <form action="{{ route('search') }}" method="GET" class="w-full flex justify-center">
      <input type="text" name="query" placeholder="Rechercher votre article..." class="w-2/5 open nav_barre">
  ...
  <div class="h-8 bg-white">
    ...
    <div><h1 class="kalam text-4xl bg-white flex justify-center">Résultats de recherche pour "{{ $query }}"</h1></div>
  ...
  <a href="{{ route('articles.index')}}">
    ...
    @else
      <div class="h-8 bg-white"></div>
        <p class="flex justify-center open text-lg bg-white ">Aucun résultat trouvé.</p>
    @endif
    <div class="h-8 bg-white"></div>
</x-app-layout>